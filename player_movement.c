...
/*   Updated: 2025/01/23 12:01:55 by porellan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player_w(t_game *game, int y, int x)
{
	...
	if (game->map[y - 1][x] == END && game->collectives != 0)
		return ;
	mlx_image_to_window(game->wndw, game->player_img, x * 64, (y - 1) * 64);
	mlx_image_to_window(game->wndw, game->floor_img, x * 64, y * 64);
	game->map[y][x] = FLOOR;
...
void	move_player_a(t_game *game, int y, int x)
{
	...
	if (game->map[y][x - 1] == END && game->collectives != 0)
		return ;
	mlx_image_to_window(game->wndw, game->player_img, (x - 1) * 64, y * 64);
	mlx_image_to_window(game->wndw, game->floor_img, x * 64, y * 64);
